...
public:
    ...
    ComPtr<ID3D11DepthStencilState>   groundDrawDSS;              // depth test on + write on / stencil write on (ground DSS)
    ComPtr<ID3D11DepthStencilState>   groundTestDSS;              // stencil test only (0x01)
    ComPtr<ID3D11DepthStencilState>   lightingVolumeDrawDSS;      // depth test only / stencil write on (stencil test ALWAYS)
